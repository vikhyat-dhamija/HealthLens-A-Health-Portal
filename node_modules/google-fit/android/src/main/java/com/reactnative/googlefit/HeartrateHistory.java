...
public class HeartrateHistory {
    ...
    public void saveBloodPressure(long startTime, long endTime, final Callback successCallback) {
        ...
        Log.i(TAG, "Wa-int: " + GoogleFitPackage.PACKAGE_NAME);
        ...
        DataSource bloodPressureSource = new DataSource.Builder()
                                        ...
                                        .setType(DataSource.DATA_QUALITY_BLOOD_PRESSURE_AAMI)
                                        .build();

        DataSet bloodPressureSet = DataSet.create(bloodPressureSource);                                   
        DataPoint bloodPressure = DataPoint.create(bloodPressureSource);
        bloodPressure.setTimestamp(date.getTime(), TimeUnit.MILLISECONDS);
        bloodPressure.getValue(HealthFields.FIELD_BLOOD_PRESSURE_SYSTOLIC).setFloat(150.0f);
        bloodPressure.getValue(HealthFields.FIELD_BLOOD_PRESSURE_DIASTOLIC).setFloat(90.0f);
        bloodPressureSet.add(bloodPressure);

        PendingResult<Status> saveDataStatus = Fitness.HistoryApi
                                        .insertData(googleFitManager.getGoogleApiClient(), bloodPressureSet);
        ...
        saveDataStatus.setResultCallback(new ResultCallback<Status>() {
            ...
            public void onResult(@NonNull Status response) {
                ...
                if (response.isSuccess()) {
                    result.putString("statusMessage", response.getStatusMessage());	
