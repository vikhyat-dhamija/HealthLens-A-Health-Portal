...
class RNGoogleFit {
  ...
  _retrieveDailySleepDataSamples = (startDate, endDate, currentDate, callback) => {
    googleFit.getDailySleepDataSamples(startDate, endDate,
      (msg) => callback(msg, false),
      (res) => {
        if (res.length > 0) {
          let response = []
          res.map((dev) => {
            ...
            let ed = new Date(dev.endDate).toLocaleDateString()
            if (dev.activity === 'sleep' && cd === ed) {
              const obj = {}
              obj.activity = dev.activity
              obj.startDate = dev.startDate
              obj.endDate = dev.endDate
              obj.difference = dev.endDate - dev.startDate
              obj.appPackageName = dev.appPackageName
              obj.identifier = dev.identifier
              return response.push(obj)
            }
          }, this)
  ...
  _retrieveSleepDataPerWeek = (startDate, endDate, callback) => {
    googleFit.getDailySleepDataSamples(startDate, endDate,
      (msg) => callback(msg, false),
      (res) => {
        if (res.length > 0) {
          ...
          for (let i = 1; i <= 7; i++) {
            ...
            const sleepDataDateList = []
            res.map(dev => {
              const endDate = new Date(dev.endDate)
              if (dev.activity === 'sleep' && currentDate.toLocaleDateString() === endDate.toLocaleDateString()){
                const obj = {}
                obj.activity = dev.activity
                obj.startDate = dev.startDate
                obj.endDate = dev.endDate
                ...
                obj.appPackageName = dev.appPackageName
                obj.identifier = dev.identifier
                sleepDataDateList.push(obj)
              }
            })
            response.push(sleepDataDateList)
          }
          callback(false, response)
        } else {
          callback('There is no any sleep data for this period', false)
  ...
  _retrieveBloodPressureDataSamples = (startDate, endDate, callback) => {
    googleFit.getBloodPressureSamples(startDate, endDate,
      (msg) => callback(msg, false),
      (res) => {
        if (res.length > 0) {   
  ...
  _saveSleepData = (startDate, endDate, callback) => {
    googleFit.saveSleepData(startDate, endDate,
      (msg) => callback(msg, false),
      (res) => {
        if (res.isSuccess) {
          callback(false, res);
  ...
  _saveBloodPressure = (startDate, endDate, callback) => {
    googleFit.saveBloodPressure(startDate, endDate,
      (msg) => callback(msg, false),
      (res) => {
        if (res.isSuccess) {
          callback(false, res);
         } else {
          callback('Blood pressure is failure', false);
         }
      }
    )
  }
  ...
  getDailySleepDataSamples = (options, callback) => {
    const startDate = !isNil(options.startDate) ? Date.parse(options.startDate) : (new Date()).setHours(0, 0, 0, 0)
    const endDate = !isNil(options.endDate) ? Date.parse(options.endDate) : (new Date()).valueOf()
    const currentDate = !isNil(options.currentDate) ? Date.parse(options.currentDate) : (new Date()).setHours(0, 0, 0, 0)
    if (!callback || typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        this._retrieveDailySleepDataSamples(startDate, endDate, currentDate, (error, result) => {
          if (!error) {
            resolve(result)
          } else {
            reject(error)
    ...
    this._retrieveDailySleepDataSamples(startDate, endDate, currentDate, callback)
  }

  getSleepPerWeek = (options, callback) => {
    const startDate = !isNil(options.startDate) ? Date.parse(options.startDate) : (new Date()).setHours(0, 0, 0, 0)
    const endDate = !isNil(options.endDate) ? Date.parse(options.endDate) : (new Date()).valueOf()
    if (!callback || typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        this._retrieveSleepDataPerWeek(startDate, endDate, (error, result) => {
          if (!error) {
            resolve(result)
          } else {
            reject(error)
    ...
    this._retrieveSleepDataPerWeek(startDate, endDate, callback)
  }


  saveSleepData = (options, callback) => {
    const startDate = !isNil(options.startDate) ? Date.parse(options.startDate) : (new Date()).setHours(0, 0, 0, 0)
    const endDate = !isNil(options.endDate) ? Date.parse(options.endDate) : (new Date()).valueOf()
    if (!callback || typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        this._saveSleepData(startDate, endDate, (error, result) => {
          if (!error) {
            resolve(result)
          } else {
            reject(error)
    ...
    this._saveSleepData(startDate, endDate, callback)
  }
  ...
  getBloodPressureSamples(options, callback) {
    ...
    if (!callback || typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        this._retrieveBloodPressureDataSamples(startDate, endDate, (error, result) => {
          if (!error) {
            resolve(result)
          } else {
            reject(error)
    ...
    this._retrieveBloodPressureDataSamples(startDate, endDate, callback)
  ...
  saveBloodPressure(options, callback) {
    ...
    if (!callback || typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        this._saveBloodPressure(startDate, endDate, (error, result) => {
          if (!error) {
            resolve(result)
          } else {
            reject(error)
    ...
    this._saveBloodPressure(startDate, endDate, callback)
  }
